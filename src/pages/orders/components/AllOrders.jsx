          ...
          <table className='w-full bg-[#fff] rounded-tr-xl rounded-tl-xl mt-4'>
                ...
                {currentData?.length > 0 ? currentData?.map((data, index) => (
                    <tr key={index} className='bg-white h-[56px] border-t cursor-pointer border-grey-100' onClick={() => navigationCheck(data)}>
                        ...
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-semibold font-Mont text-dark-100 text-left'>{`#${data?.txn_id?.substring(0, 8)}`}</p> 
                        </td>
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-Mont text-dark-100 text-left'>{timeAgo(data?.created_at)}</p>
                        </td>
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-Mont text-dark-100 text-left'>{data?.items_count || "N/A"}</p>
                        </td>
                        <td className='h-[70px] px-4'>
                            <p className='text-sm font-Mont text-dark-100 text-left'>{`₦${data?.total_amount}`}</p>
                        </td>
                        <td className='h-[70px] px-4'>
                            <div className={`rounded-lg h-8 flex justify-center items-center ${data.status === 'pending' && 'w-[99px]  bg-[#FFC60029]'} ${data.status === 'Enroute' && ' w-[99px] bg-[#000]'} ${data.status === 'delivered' && ' w-[99px] bg-[#ECFDF5]'} ${data.status === 'Cancelled' && 'w-[92px] bg-[#FFF1F2]'} `}>
                                <p className={`text-sm font-Mont text-left font-semibold ${data.status === 'pending' && 'text-[#FFC600]'} ${data.status === 'Enroute' && 'text-[#FFF]'} ${data.status === 'delivered' && 'text-[#10B981]'} ${data.status === 'Cancelled' && 'text-[#F43F5E]'} `}>
                                  {data.status === "pending" ? "Processing" : data.status === "Enroute" ? "Enroute" : data.status === "delivered" ? "Delivered" : data.status}
                                </p>
                            </div>
                        </td>
                    </tr>
