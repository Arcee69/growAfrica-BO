...
const OrderDetails = () => {
    ...
    const getOrderById = async () => {
        await api.get(`${appUrls?.GET_ORDER_URL}/${state.id}`)
        .then((res) => {
            console.log(res, "asap")
            setItemDetails(res?.data?.data?.order)
        })
        .catch((err) => {
    ...
    <div className='p-8'>
        ...
        <div className='flex flex-col gap-2 mt-6'>
            <div className='flex items-center gap-2'>
                ...
                <div className={`rounded-lg h-8 flex justify-center items-center ${state?.status === 'pending' && 'w-[78px]  bg-[#FFC60029]'} ${state?.status === 'accepted' && ' w-[99px] bg-[#ECFDF5]'} ${state?.status === 'returned' && 'w-[92px] bg-[#FFF1F2]'} `}>
                    <p className={`text-sm font-Mont text-left ${state?.status === 'pending' && 'text-[#FFC600]'} ${state?.status === 'shipped' && 'text-[#10B981]'} ${state?.status === 'Enroute' && 'text-[#FFC600]'} ${state?.status === 'Cancelled' && 'text-[#F43F5E]'} `}>
                        {state?.status === "pending" ? "Processing" : state?.status === "shipped" ? "Shipped" : state?.status === "Enroute" ? "Enroute" : state?.status === "returned" ? "Returned" : "Delivered"}
                    </p>
                </div>
                <button 
                    type='button'
                    className={`${state?.status === "delivered" ? "hidden" : "w-[159px] h-[38px] bg-[#000] text-center text-[#fff] rounded"}`}
        ...
        {activeTab === "Details" && <Details itemDetails={itemDetails} />}
