...
const AddImages = ({ handleClose, productData }) => {
    ...
    try {
        const res = await api.post(appUrls?.UPLOAD_IMAGE_URL_A, formData);
        const resB = await api.post(appUrls?.UPLOAD_IMAGE_URL_B, formDataB);
        const resC = await api.post(appUrls?.UPLOAD_IMAGE_URL_C, formDataC);
        console.log(res, "pop");
        setLoading(false)
